...
import com.zr.constant.SysConstant;
...
public class RestfulFilter extends UserFilter {
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        if (StringUtils.isBlank(loginToken)) {// 非Restful方式
            ...
            if (!httprequest.getRequestURI().contains(".do") || SysConstant.urlExcloud.contains(httprequest.getRequestURI())) {
    ...
    private static String info = JSONObject
            .toJSONString(new ResponseBean(HttpStatus.UNAUTHORIZED.value() + "", "token不存在或者过期"));
